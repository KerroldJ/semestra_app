...
1. **Foundation** — bundle fonts + rewrite `AppTheme` (gold/warm, `tnum`).
2. **Auth module** — entity, `user_profile` table (DB v6), `AuthService`,
   `authNotifierProvider`.
3. **Router gate** — auth redirect + new 4-tab shell + compose ring.
4. **Onboarding (01–05)** — welcome, sign-in, username, subjects, class times.
5. **Core tabs** — Today (06), Planner (07–08), Subjects (13–14), Notes (15–16).
6. **Secondary** — Compose sheet (09), Assignments (10–12), Semester (17),
   Settings (18).
7. **Cleanup + tests** — remove workspace/progress/profile dirs, add tests,
   `flutter analyze` + `flutter test`, manual run.
